...
export async function DELETE(
  ...
  try {
    ...
    const response = await fetch(`http://localhost:3000/api/lists/${resolvedParams.id}/books/${resolvedParams.bookId}`, {
      method: 'DELETE',
      headers,
    });
    ...
    console.error('Proxy remove book from list error:', error);
    return NextResponse.json(
